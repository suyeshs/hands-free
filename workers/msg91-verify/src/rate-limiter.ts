/**
 * Rate Limiter for MSG91 Verification Requests
...
export class RateLimiter {
   ...
   */
  async getRateLimitInfo(phoneNumber: string): Promise<RateLimitInfo & { attemptsRemaining: { hourly: number; daily: number } }> {
    ...
    return {
      allowed: !blocked && hourCount < MAX_ATTEMPTS_PER_HOUR && dayCount < MAX_ATTEMPTS_PER_DAY,
      attemptsRemaining: {
        hourly: Math.max(0, MAX_ATTEMPTS_PER_HOUR - hourCount),
        daily: Math.max(0, MAX_ATTEMPTS_PER_DAY - dayCount),
      },
    };
  }
