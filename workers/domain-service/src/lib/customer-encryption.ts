...
export async function getTenantEncryptionKey(
  ...
  try {
    const response = await tokenManager.fetch(
      new Request(`https://token-manager/api/secrets/${encodeURIComponent(keyName)}`, {
        method: 'GET',
        headers: {
    ...
    const data = await response.json() as { key_value: string; version?: number };
    return {
      key: data.key_value,
...
export async function encryptCustomerPII(
  ...
  const encrypted: EncryptedCustomerPII = {
    phone_hash: hashPhoneNumber(pii.phone || '', tenantId),
