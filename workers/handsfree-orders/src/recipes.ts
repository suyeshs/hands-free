 ...
 *
 * Uses Gemini 1.5 Flash to automatically generate recipe ingredients from menu items
 * by matching them to existing inventory items.
 */
...
/**
 * Match an ingredient name to inventory items
...
- Available inventory items: ${inventoryItems.length}
...
async function getInventoryItems(tenantId: string, env: Env): Promise<InventoryItem[]> {
  try {
    ...
    const response = await fetch(queryUrl, {
      ...
      body: JSON.stringify({
        sql: 'SELECT id, name, category, unit FROM inventory_items WHERE tenant_id = ? ORDER BY name',
        params: [tenantId],
      }),
    });
