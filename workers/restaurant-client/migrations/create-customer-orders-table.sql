...
-- Create trigger to update menu_items order_count when order is confirmed
CREATE TRIGGER IF NOT EXISTS update_order_count_on_order
AFTER INSERT ON customer_orders
WHEN NEW.status = 'confirmed'
BEGIN
  -- This would ideally parse order_items JSON and increment order_count
  -- However, SQLite doesn't support JSON functions easily, so this can be handled in application code
  NULL;
END;
