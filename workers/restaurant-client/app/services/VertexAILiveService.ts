...
export class VertexAILiveService {
  ...
  /**
   * Sync manual cart updates to backend
   */
  syncCartUpdate(action: 'add' | 'update' | 'remove', item: {
    dishName: string;
    quantity: number;
    customization?: string;
    itemId?: string;
  }): void {
    // Serialize to plain object (handles MobX observables)
    const plainItem = JSON.parse(JSON.stringify(item));
    console.log('[VertexAILive] Syncing manual cart update:', action, plainItem);
    this.sendMessage({
      type: 'manual_cart_update',
      action,
      item: plainItem
    });
  }
