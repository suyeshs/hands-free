/**
 * Bar Display System V2 - Industrial Redesign
 * "Idiot Proof" Industrial BDS design - Full screen, high visibility
...
import { BDSFloatingOrb } from '../components/bds/BDSFloatingOrb';
import { BDSGroupedOrderCard } from '../components/bds/BDSGroupedOrderCard';
...
export default function BarDashboard() {
  ...
  // Theme state (default to dark for bar environment)
  ...
  const handleItemStatusChange = async (orderId: string, itemId: string, newStatus: BarItemStatus) => {
    try {
  ...
  const handleMarkOutOfStock = (order: BarOrder, item: BarOrderItem) => {
    setSelectedOosItem({
      itemName: item.name,
      orderId: order.id,
  ...
  const getOrderAge = (order: BarOrder): number => {
    const startTime = new Date(order.createdAt).getTime();
    return Math.floor((currentTime - startTime) / 1000 / 60);
  };

  // Get urgency level (bar orders are faster - 10/15 min thresholds instead of 15/20)
  ...
  // Show all orders (no station filtering needed for BDS)
  ...
  const runningOrdersCount = activeOrders.filter((o: BarOrder) => o.isRunningOrder).length;

  // Stats
  const activeOrdersCount = activeOrders.filter((o: BarOrder) => o.status !== 'completed').length;
  const pendingItems = activeOrders.reduce(
    (sum: number, o: BarOrder) => sum + o.items.filter((i: BarOrderItem) => i.status === 'pending').length,
    0
  );
  const urgentOrders = activeOrders.filter((o: BarOrder) => getOrderAge(o) > 15).length;
      ...
      <button
        ...
        className={cn(
          ...
          activeTab === 'active'
            ...
            : theme === 'dark'
              ? 'bg-slate-900/50 border-slate-700 text-slate-400 hover:text-white hover:bg-purple-900/50'
    ...
    <div className={cn(
      ...
      {(isMobile || isSmallTablet || isTablet) && (
        <header className={cn(
          ...
          theme === 'dark' ? 'bg-black border-purple-800' : 'bg-white border-gray-300'
        )}>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <h1 className={cn(
                ...
                theme === 'dark' ? 'text-purple-400' : 'text-gray-900',
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className={cn(
                  ...
                  theme === 'dark'
                    ? 'bg-purple-800 text-yellow-400 hover:bg-purple-700'
              ...
              {showStats && (
                <div className="flex gap-2">
                  <div className="bg-purple-900 border border-purple-700 rounded px-2 py-1 flex items-center gap-1">
                    <span className="font-black text-purple-400 text-sm">{activeOrdersCount}</span>
        ...
        <>
          <div className={cn(
            ...
            <div className="flex items-center gap-6">
              ...
              <button
                ...
                className={cn(
                  ...
                  theme === 'dark'
                    ? 'bg-purple-800 border-purple-700 text-yellow-400 hover:bg-purple-700'
            ...
            <div className="flex gap-4">
              <div className="bg-purple-900 border-2 border-purple-700 px-6 py-2 rounded flex flex-col items-center min-w-[120px]">
                <span className="text-xs text-purple-300 font-bold uppercase tracking-wider">Active</span>
                <span className="text-3xl font-black text-purple-400">{activeOrdersCount}</span>
              </div>
              <div className="bg-purple-900 border-2 border-purple-700 px-6 py-2 rounded flex flex-col items-center min-w-[120px]">
                <span className="text-xs text-purple-300 font-bold uppercase tracking-wider">Pending</span>
              ...
              <div className={cn("bg-purple-900 border-2 border-purple-700 px-6 py-2 rounded flex flex-col items-center min-w-[120px]", urgentOrders > 0 && "animate-pulse border-red-500 bg-red-900/20")}>
                <span className="text-xs text-purple-300 font-bold uppercase tracking-wider">Urgent</span>
              ...
              <button
                ...
                className={cn(
                  ...
                  oosItemsCount > 0
                    ...
                    : "bg-purple-900 border-2 border-purple-700 hover:bg-purple-800"
                )}
              >
                <span className={cn(
                  "text-xs font-black uppercase tracking-wider",
                  oosItemsCount > 0 ? "text-red-400" : "text-purple-300"
                )}>86'd</span>
                <span className={cn(
                  "text-3xl font-black",
          ...
          <>
            {filteredOrders.length === 0 ? (
              ...
              <div className={cn(
                ...
                {filteredOrders.map((order: BarOrder) => {
          ...
          <>
            {sortedCompletedOrders.length === 0 ? (
              ...
              <div className={cn(
                ...
                {sortedCompletedOrders.map((order: BarOrder) => {
