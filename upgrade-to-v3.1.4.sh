...
# Coorg Food Company - Upgrade to v3.1.4
# Creates clean guanix.db with all existing data + v3.1.4 plugin system

set -e

SOURCE_DB="/Users/stonepot-tech/projects/restaurant-pos-ai/pos.db"
TARGET_DB="/Users/stonepot-tech/projects/restaurant-pos-ai/guanix.db"
BACKUP_DIR="$HOME/Desktop/coorg-migration-backups"

echo "🚀 Coorg Food Company - Upgrade to v3.1.4"
echo "=========================================="
echo ""
...
cp "$SOURCE_DB" "$BACKUP_DIR/pos-backup-$timestamp.db"
echo "✅ Backup created: $BACKUP_DIR/pos-backup-$timestamp.db"
echo ""

# Stage 2: Analyze source
echo "📊 Stage 2: Analyzing source database..."
...
# Stage 3: Create new database with v3.1.4 schema
echo "🔨 Stage 3: Creating guanix.db with v3.1.4 schema..."
if [ -f "$TARGET_DB" ]; then
    mv "$TARGET_DB" "$BACKUP_DIR/guanix-old-$timestamp.db"
    echo "   ⚠️  Existing guanix.db backed up"
fi
...
VALUES (1, 'upgrade_to_v3.1.4', 'upgrade-script', '3.1.4');
...
# Stage 4: Copy existing data
echo "📥 Stage 4: Copying existing data..."

# Copy existing sales_transactions (364 records)
echo "   Copying existing sales_transactions..."
...
# Stage 5: Migrate old table_sessions (8 records)
if [ "$old_sessions" -gt 0 ]; then
    echo "🔄 Stage 5: Migrating $old_sessions old table_sessions to sales_transactions..."
...
EOF
    ...
    echo "⏭️  Stage 5: No table_sessions to migrate"
fi
echo ""
...
echo "   Final counts in guanix.db:"
...
echo ""
echo "Summary:"
echo "  ✅ Backup: $BACKUP_DIR/pos-backup-$timestamp.db"
...
echo ""
echo "Next steps:"
echo "  1. Review the migrated data in guanix.db"
echo "  2. Install and launch v3.1.4 POS application"
echo "  3. Verify all data loads correctly"
echo "  4. Enable cloud sync when ready"
